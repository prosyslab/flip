...
def inject_error(project, case, env_signal_list, neg_num):
    ...
    # run docker
    cmd = [f'{RUN_DOCKER_SCRIPT}', f'{project}-{case}', '-d', '--rm', '--name', f'{project}-{case}-errorcall']
    cmd = cmd + ['--mem', '40g'] if project == 'python' else cmd
    run_cmd_and_check(cmd,
                      stdout=subprocess.DEVNULL,
                      stderr=subprocess.DEVNULL)

    docker_id = f'{project}-{case}-errorcall'
    sleep(5)
    ...
    for (inject_file, inject_line, _) in env_signal_list:
        for i in range(neg_num):
            test_num = i+1
            os.makedirs(COVERAGE_DIR / project / case / 'branch' / inject_file / inject_line / 'call', exist_ok=True)
            
            run_cmd_and_check(
                ['timeout', '1m', 'docker', 'exec', '-w', '/experiment', docker_id, 'bash', '-c', f'./run.sh {inject_file}:{inject_line} n{test_num} || true'],
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL)
            ...
            run_cmd_and_check(
                ['docker', 'cp', f'{docker_id}:/experiment/coverage_data/coverage.txt', f"{str(COVERAGE_DIR / project / case / 'branch' / inject_file / inject_line / 'call' / ('n' + str(test_num) + '.txt'))}"],
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL)
